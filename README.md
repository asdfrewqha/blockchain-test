...
> Для него вам нужно все так же записать переменные окружения в `.env`, но переместить его на сервер, а также дописать необходимую информацию в секреты репозитория. После такой настройки вы сможете любым комитом в `main` запустить автоматическую загрузку приложения на сервер.
